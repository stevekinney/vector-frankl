...
describe('WASMOperations', () => {
  ...
  describe('Dot Product Operations', () => {
    ...
    it('should handle large vectors efficiently', async () => {
      ...
      const vectorB = new Float32Array(Array.from({ length: size }, (_, i) => (i + 1) * 2));
    ...
    it('should throw error for mismatched dimensions', async () => {
      ...
      expect(wasmOps.dotProduct(vectorA, vectorB)).rejects.toThrow('Vector dimensions must match');
    });
  });
  ...
  describe('Vector Addition', () => {
    ...
    it('should throw error for mismatched dimensions', async () => {
      ...
      expect(wasmOps.vectorAdd(vectorA, vectorB)).rejects.toThrow('Vector dimensions must match');
    });
  });
