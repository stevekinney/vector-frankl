    ...
    await page.evaluate(async () => {
      try {
        ...
        if (retrieved.vector.length !== dimension) {
          throw new Error(
            `Vector dimension mismatch: expected ${dimension}, got ${retrieved.vector.length}`,
          );
        }
        ...
        window.log(
          `Large vector (${dimension}D): add=${addTime.toFixed(2)}ms, retrieve=${retrievalTime.toFixed(2)}ms, search=${searchTime.toFixed(2)}ms`,
        );
    ...
    await page.evaluate(async () => {
      try {
        ...
        const missingStores = expectedStores.filter(
          (store) => !objectStoreNames.includes(store),
        );
  ...
  test('should handle concurrent database operations', async ({ page }) => {
    ...
    await page.evaluate(async () => {
      try {
        ...
        window.log(
          `Completed ${numOperations} concurrent writes in ${writeTime.toFixed(2)}ms`,
        );
        ...
        if (concurrentVectors.length !== numOperations) {
          throw new Error(
            `Expected ${numOperations} concurrent vectors, got ${concurrentVectors.length}`,
          );
        }
        ...
        window.log(
          `Completed ${successfulReads}/${numOperations} concurrent reads in ${readTime.toFixed(2)}ms`,
        );

        if (successfulReads !== numOperations) {
          throw new Error(
            `Some concurrent reads failed: ${successfulReads}/${numOperations}`,
          );
        }
    ...
    await page.evaluate(async () => {
      try {
        ...
        if ('storage' in navigator && 'estimate' in navigator.storage) {
          ...
          window.log(
            `Storage after adding ${testCount} vectors: ${newUsageMB}MB (${newUsageMB - usageMB}MB increase)`,
          );
          ...
          window.addTestResult(
            'Storage Quota',
            'success',
            'Storage API not available (browser limitation)',
          );
        }
      } catch (error) {
  ...
  test('should handle database errors gracefully', async ({ page }) => {
    ...
    await page.evaluate(async () => {
      try {
        ...
        if (errorCount >= 3) {
          // At least 3 out of 4 error cases handled
          window.addTestResult(
            'Error Handling',
            'success',
