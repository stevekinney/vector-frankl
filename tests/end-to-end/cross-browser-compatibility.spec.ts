...
test.describe('Cross-Browser Compatibility Tests', () => {
  test('should work across all browsers', async ({ page, browserName }) => {
    ...
    await expect(page.locator('#db-status')).toContainText('Initialized', { timeout: 15000 });
    ...
    await page.evaluate(async (browser) => {
      try {
        ...
        window.addTestResult(`${browser} Compatibility`, 'success', 
  ...
  test('should detect browser capabilities correctly', async ({ page, browserName }) => {
    ...
    await page.evaluate(({ caps, browser }) => {
      ...
      const supportScore = [
        caps.indexedDB, caps.webWorkers, caps.webAssembly, caps.webGL, caps.cryptoAPI, caps.performanceAPI
      ].filter(Boolean).length;
      
      window.addTestResult(`${browser} Feature Support`, 'success', 
  ...
  test('should handle browser-specific storage limits', async ({ page, browserName }) => {
    ...
    await page.evaluate(async (browser) => {
      try {
        ...
        window.log(`${browser}: Retrieval: ${retrievalTime.toFixed(2)}ms, Search: ${searchTime.toFixed(2)}ms`);
        ...
        window.addTestResult(`${browser} Storage Limits`, 'success', 
  ...
  test('should handle browser-specific API differences', async ({ page, browserName }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    await page.evaluate(async (browser) => {
      try {
        ...
        window.addTestResult(`${browser} API Compatibility`, 'success', 
          `${workingAPIs}/${totalAPIs} APIs functional: ${Object.entries(apiTests).filter(([,works]) => works).map(([api]) => api).join(', ')}`);
      } catch (error) {
    ...
    await expect(page.locator('#test-results')).toContainText('API Compatibility: SUCCESS');
  });
});