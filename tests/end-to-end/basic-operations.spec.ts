...
test.describe('Vector Database Basic Operations', () => {
    ...
    await page.evaluate(async () => {
      try {
        ...
        const testVectors = [
          {
            ...
            metadata: { category: 'test', type: 'A' },
          },
          {
            id: 'vec2',
            vector: new Array(384).fill(0).map(() => Math.random()),
            metadata: { category: 'test', type: 'B' },
          },
          {
            id: 'vec3',
            vector: new Array(384).fill(0).map(() => Math.random()),
            metadata: { category: 'demo', type: 'A' },
          },
        ];
        ...
        if (allVectors.length !== 3)
          throw new Error(`Expected 3 vectors, got ${allVectors.length}`);
        window.log(`Listed ${allVectors.length} vectors`);

        // UPDATE: Update metadata
        await window.db.updateMetadata('vec1', {
          category: 'updated',
          type: 'A',
          timestamp: Date.now(),
        });
        const updated = await window.db.getVector('vec1');
        if (updated.metadata.category !== 'updated')
        ...
        if (remaining.length !== 2)
          throw new Error(`Expected 2 vectors after deletion, got ${remaining.length}`);
  ...
  test('should handle vector search with different distance metrics', async ({
    page,
  }) => {
    // Initialize database first
    await page.click('#init-db');
    ...
    await page.evaluate(async () => {
      try {
        ...
        if (allVectors.length < batchSize) {
          throw new Error(
            `Expected at least ${batchSize} vectors, got ${allVectors.length}`,
          );
        }

        window.log(
          `Retrieved ${allVectors.length} vectors in ${retrievalTime.toFixed(2)}ms`,
        );
