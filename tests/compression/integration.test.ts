...
describe('Compression Integration', () => {
  ...
  describe('Auto-Selection Logic', () => {
    it('should recommend scalar for general vectors', () => {
      const generalVector = new Float32Array(
    ...
    it('should handle small vectors appropriately', () => {
      const smallVector = new Float32Array(
    ...
    it('should detect high dynamic range vectors', () => {
      const highRangeVector = new Float32Array(
        Array.from(
          { length: 200 },
          (_, i) => (i < 100 ? 0.001 * i : 1000 * (i - 100)), // Create a larger vector with high range
        ),
      );
  ...
  describe('Real-World Scenarios', () => {
    ...
    it('should handle high-precision scientific data', async () => {
      ...
      try {
        ...
        for (let i = 0; i < scientificData.length; i++) {
          const relativeError =
            Math.abs(scientificData[i]! - decompressed[i]!) /
            Math.abs(scientificData[i]!);
