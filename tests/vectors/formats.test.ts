...
describe('VectorFormatHandler Enhanced Validation', () => {
  describe('Format Detection', () => {
    it('should detect valid formats correctly', () => {
      expect(VectorFormatHandler.detectFormat(new Float32Array([1, 2, 3]))).toBe('float32');
      expect(VectorFormatHandler.detectFormat(new Float64Array([1, 2, 3]))).toBe('float64');
    ...
    it('should throw error for null/undefined vectors', () => {
      expect(() => VectorFormatHandler.detectFormat(null as any)).toThrow(InvalidFormatError);
      expect(() => VectorFormatHandler.detectFormat(undefined as any)).toThrow(InvalidFormatError);
    });

    it('should throw error for empty vectors', () => {
      expect(() => VectorFormatHandler.detectFormat(new Float32Array([]))).toThrow(InvalidFormatError);
    ...
    it('should throw error for arrays with non-numeric values', () => {
      expect(() => VectorFormatHandler.detectFormat([1, 'invalid', 3] as any)).toThrow(InvalidFormatError);
  ...
  describe('Enhanced Validation', () => {
    it('should validate dimension constraints', () => {
      ...
      expect(() => VectorFormatHandler.validate(vector, 4)).toThrow(DimensionMismatchError);
    });
    ...
    it('should validate dimension ranges', () => {
      ...
      expect(() => VectorFormatHandler.validate(vector, undefined, { 
        minDimension: 3, 
        maxDimension: 10 
      })).not.toThrow();
    ...
    it('should validate normalization requirement', () => {
      ...
      expect(() => VectorFormatHandler.validate(normalizedVector, undefined, { 
        requireNormalized: true 
      })).not.toThrow();
      
      // Should fail for unnormalized vector
      expect(() => VectorFormatHandler.validate(unnormalizedVector, undefined, { 
        requireNormalized: true 
    ...
    it('should validate NaN and infinity constraints', () => {
      ...
      expect(() => VectorFormatHandler.validate(vectorWithNaN)).toThrow('Vector contains NaN values');
      
      // Should reject Infinity by default
      expect(() => VectorFormatHandler.validate(vectorWithInfinity)).toThrow('Vector contains infinite values');
      
      // Should allow NaN when explicitly enabled
      expect(() => VectorFormatHandler.validate(vectorWithNaN, undefined, { 
        allowNaN: true 
      })).not.toThrow();
      
      // Should allow Infinity when explicitly enabled
      expect(() => VectorFormatHandler.validate(vectorWithInfinity, undefined, { 
        allowInfinity: true 
      })).not.toThrow();
    });
  ...
  describe('Vector Utilities', () => {
    ...
    it('should throw error when normalizing zero vector', () => {
      ...
      expect(() => VectorFormatHandler.normalize(zeroVector)).toThrow('Cannot normalize zero vector');
    });
    ...
    it('should suggest optimal formats correctly', () => {
      ...
      const fullPrecisionSuggestion = VectorFormatHandler.suggestOptimalFormat(fullPrecisionVector);
      expect(fullPrecisionSuggestion.recommendedFormat).toBe('float32');
      expect(fullPrecisionSuggestion.reasoning).toContain('full floating-point precision');
    });
  });
  ...
  describe('Format Conversion', () => {
    ...
    it('should handle edge cases in format conversion', () => {
      ...
      const asInt8 = VectorFormatHandler.fromFloat32Array(extremeValues, 'int8') as Int8Array;
