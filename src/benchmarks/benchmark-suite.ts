...
export class BenchmarkSuite {
   ...
   */
  @debugMethod('benchmark.runSuite', 'basic', { profileEnabled: true, memoryTracking: true })
    ...
    for (const dimension of this.config.dimensions) {
      for (const size of this.config.datasetSizes) {
        ...
        await this.runBenchmark(
          ...
          'database-ops',
          async () => {
            const batchVectors = vectors.slice(0, Math.min(100, size)).map((vector, i) => ({
              id: `batch-${i}`,
              vector
            }));
  ...
  private async benchmarkVectorFormats(): Promise<void> {
    ...
    const formats: Array<{ name: string; generator: (dim: number) => VectorFormat }> = [
      { name: 'Float32Array', generator: (dim) => new Float32Array(this.generateRandomVector(dim)) },
      { name: 'Float64Array', generator: (dim) => new Float64Array(this.generateRandomVector(dim)) },
      { name: 'Array', generator: (dim) => this.generateRandomVector(dim) },
      { name: 'Int8Array', generator: (dim) => new Int8Array(dim).map(() => Math.floor(Math.random() * 256) - 128) },
      { name: 'Uint8Array', generator: (dim) => new Uint8Array(dim).map(() => Math.floor(Math.random() * 256)) }
    ];
  ...
  private async runBenchmark(
    ...
    try {
      const startMemory = typeof process !== 'undefined' && process.memoryUsage ? process.memoryUsage() : undefined;
      ...
      const endMemory = typeof process !== 'undefined' && process.memoryUsage ? process.memoryUsage() : undefined;
    ...
    if (this.config.verbose) {
      const status = error ? '❌' : '✅';
      console.log(`${status} ${testName}: ${operationsPerSecond.toFixed(2)} ops/sec (${avgDuration.toFixed(2)}ms)`);
      if (error) {
  ...
  private generateSummary(): BenchmarkSummary {
    ...
    const categories: Record<string, { averageOps: number; totalDuration: number; testCount: number }> = {};
  ...
  private generateRecommendations(results: BenchmarkResult[]): string[] {
    ...
    if (searchResults.length > 0) {
      const avgSearchOps = searchResults.reduce((sum, r) => sum + r.operationsPerSecond, 0) / searchResults.length;
      
      if (avgSearchOps < 100) {
        recommendations.push('Consider enabling HNSW indexing to improve search performance');
      }
      
      if (avgSearchOps > 1000) {
        recommendations.push('Excellent search performance - current configuration is well-optimized');
      }
    }
    ...
    if (indexedSearches.length > 0 && linearSearches.length > 0) {
      const indexedAvg = indexedSearches.reduce((sum, r) => sum + r.operationsPerSecond, 0) / indexedSearches.length;
      const linearAvg = linearSearches.reduce((sum, r) => sum + r.operationsPerSecond, 0) / linearSearches.length;
      
      const speedup = indexedAvg / linearAvg;
      if (speedup > 2) {
        recommendations.push(`HNSW indexing provides ${speedup.toFixed(1)}x speedup - recommended for large datasets`);
      }
    }
    ...
    if (memoryResults.length > 0) {
      const totalMemoryUsed = memoryResults.reduce((sum, r) => sum + (r.memoryUsage?.heapUsed || 0), 0);
      if (totalMemoryUsed > 100 * 1024 * 1024) { // 100MB
        recommendations.push('Consider enabling compression for large datasets to reduce memory usage');
      }
    }
    
    // Analyze dimensional performance
    const highDimResults = results.filter(r => (r.metadata['dimension'] as number) > 512);
    if (highDimResults.length > 0) {
      const avgOps = highDimResults.reduce((sum, r) => sum + r.operationsPerSecond, 0) / highDimResults.length;
      if (avgOps < 50) {
        recommendations.push('High-dimensional vectors show reduced performance - consider dimensionality reduction');
      }
    }
