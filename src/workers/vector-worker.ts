  ...
  try {
    ...
    switch (operation) {
      case 'similarity_search':
        result = await performSimilaritySearch(data as Parameters<typeof performSimilaritySearch>[0]);
        break;
        
      case 'batch_similarity':
        result = await performBatchSimilarity(data as Parameters<typeof performBatchSimilarity>[0]);
        break;
...
async function performSimilaritySearch(data: {
  ...
  const results: Array<{ id: string; distance: number; score: number; metadata?: Record<string, unknown> }> = [];
  ...
  for (const vector of vectors) {
    ...
    const result: { id: string; distance: number; score: number; metadata?: Record<string, unknown> } = {
      id: vector.id,
      distance,
      score
    };
 ...
 */
function quantizeVectors(data: {
  vectors: Float32Array[];
  bits: number;
}): { quantized: Int8Array[]; scales: number[] } {
...
async function performSharedSimilaritySearch(data: {
  ...
  const { sharedBuffer, dimension, queryVector, queryOffset, k, metric, startIdx, endIdx, layout } = data;
  ...
  const isQuantized = layout && sharedBuffer.byteLength < (layout.vectorCount * dimension * 4);
  ...
  for (let i = startIdx; i < endIdx; i++) {
    ...
    if (isQuantized) {
      ...
      const quantizedData = new Int8Array(sharedBuffer, dataOffset + i * dimension, dimension);
